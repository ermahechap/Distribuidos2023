...
int main(void) {
  ...
  fftw_execute(plan);
  //Transform result to full freq spectrum (aka. add the conjugate)
  // for (int i = (N/2) + 1; i < N; i++){
  //   data_ft[i][0] = data_ft[N - i][0];
  //   data_ft[i][1] = -data_ft[N - i][1];
  // }
  ...
  // fftw_plan inverse_plan = fftw_plan_dft_1d(N, unshifted, embedded_signal, FFTW_BACKWARD, FFTW_ESTIMATE);
